...
class TDXNodeScanner:
    ...
    # ==================== 兼容性方法 ====================
    def scan_fastest(self, top_n: int = 5, async_mode: bool = False) -> List[Dict]:
        """
        获取最快的N个节点（兼容旧接口）
        
        Args:
            top_n: 返回节点数量
            async_mode: 是否使用异步模式（Windows 建议 False）
        
        Returns:
            节点列表，按延迟升序排序
        """
        if async_mode:
            results = asyncio.run(self.scan_async())
        else:
            results = self.scan_threaded()
        return [r for r in results if r["status"] == "ok"][:top_n]


# ==================== 兼容性别名 ====================
# 重要：为了兼容其他模块的导入，添加别名
NodeScanner = TDXNodeScanner
...
class TestTDXScanner(unittest.TestCase):
    ...
    def test_node_scanner_alias(self) -> None:
        """验证 NodeScanner 别名存在"""
        self.assertIs(NodeScanner, TDXNodeScanner)
...
if __name__ == "__main__":
    ...
    for i, node in enumerate(get_fastest_nodes(5, async_mode=False), 1):

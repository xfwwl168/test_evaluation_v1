...
class DataUpdater:
    ...
    def full_update(
        ...
        # 一次性合并写入
        if all_data:
            self.logger.info(f"       Merging {len(all_data)} stocks into single DataFrame...")
            combined_df = pd.concat(all_data, ignore_index=True)

            self.logger.info(f"       Writing {len(combined_df):,} rows to database...")
            self.db.upsert(combined_df)

            written = len(all_data)
